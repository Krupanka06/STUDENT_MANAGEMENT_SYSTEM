/*
* Student Management REST API Server - Enhanced Version
* Features: Multi-role system (Admin, Principal, Teacher, Student)
*           Teacher registration with Principal approval
*           Student and Teacher management with role-based access
* Compile: gcc -o student_server student_server.c -lws2_32
*/
...
#define PRINCIPAL_DEFAULT_PASSWORD "principal123"
#define TEACHER_DEFAULT_PASSWORD "teacher123"
#define DEFAULT_STUDENT_PASSWORD "student123"

// Role enumeration
typedef enum { ROLE_STUDENT = 1, ROLE_TEACHER = 2, ROLE_PRINCIPAL = 3, ROLE_ADMIN = 4 } UserRole;
...
typedef struct {
    Student* head;
    Teacher* teacherHead;
    Principal* principalHead;
    int nextStudentId;
    int nextTeacherId;
    int nextPrincipalId;
} SystemData;

SystemData g_system = {0};
...
Teacher* findTeacherByName(char* name);
Principal* findPrincipal(int principalId);
